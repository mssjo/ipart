...
def main(argv):
    ...
    if write_std or args.verbose:
        for part in rev(parts, args.reverse_output):
            print(' '.join(str(p) for p in rev(part, args.reverse)))
    if args.txt:
        with open(f"{filename}.txt", 'w') as out:
            for part in rev(parts, args.reverse_output):
                print(' '.join(str(p) for p in rev(part, args.reverse)), file=out)
        if args.verbose:
    ...
    if args.form:
        with open(f"{filename}.hf", 'w') as out:
            ...
            for part in rev(parts, args.reverse_output):
                print(f"   + ipart({','.join(str(p) for p in rev(part, args.reverse))})", file=out)
        if args.verbose:
